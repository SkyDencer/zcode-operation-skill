...
| **P6-H9** | **Generated index stores absolute personal paths.** `data/skill-index.json` entries carry `C:\Users\<user>\…\SKILL.md`. The file is git-ignored (`.gitignore:10`), but the artifact is non-portable and `readSkillContent` (`src/core/retriever/bm25.mjs:105-115`) silently skips any file it can no longer resolve. | `node -e "...console.log(JSON.parse(fs.readFileSync('data/skill-index.json'))[0].path)"` → `C:\\Users\\PC-1\\Desktop\\projects\\zcode-operation-skill\\data\\skills\\backend\\laravel\\api-resources\\SKILL.md`; `git check-ignore -v data/skill-index.json` → `.gitignore:10`. |
